...
/**
 * Component to apply JSON Patch and JSON Merge Patch payloads to existing
 * domain objects. The implementation uses the JSON Patch library by Francis
 * Galiegue but applies a customization to remove operations. As the patched
 * node set needs to be applied to the existing domain objects (see
 * {@link DomainObjectReader} for that) we turn remove calls into
 * 
 * @author Oliver Gierke
 * @author Mathias Düsterhöft
 * @see <a href=
 *      "http://tools.ietf.org/html/rfc6902">http://tools.ietf.org/html/rfc6902</a>
 * @see <a href=
...
class JsonPatchHandler {
	...
	/**
	 * Creates a new {@link JsonPatchHandler} with the given {@link ObjectMapper}
	 * and {@link DomainObjectReader}.
	 * 
	 * @param mapper
	...
	/**
	 * Applies the body of the given {@link IncomingRequest} as patch on the given
	 * target object.
	 * 
	 * @param request
	 *            must not be {@literal null}.
	...
	public <T> T apply(IncomingRequest request, T target) throws Exception {
		if(request.getServerHttpRequest().getMethod().equals(HttpMethod.PUT)) {
	 ...
	 *             in case the payload can't be read.
	 */
