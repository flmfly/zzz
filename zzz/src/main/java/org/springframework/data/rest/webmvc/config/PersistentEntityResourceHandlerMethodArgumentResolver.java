...
public class PersistentEntityResourceHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
	 ...
	 * for the given {@link HttpMessageConverter}s and
	 * {@link RootResourceInformationHandlerMethodArgumentResolver}..
	 * 
	 * @param messageConverters
	 *            must not be {@literal null}.
	 * @param resourceInformationResolver
	 *            must not be {@literal null}.
	 * @param idResolver
	 *            must not be {@literal null}.
	 * @param reader
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		for (HttpMessageConverter converter : messageConverters) {
			...
			Optional<Object> entityIdentifier = objectToUpdate
					.map(it -> entity.getIdentifierAccessor(it).getIdentifier());
	...
	/**
	 * Reads the given {@link ServerHttpRequest} into an object of the type of the
	 * given {@link RootResourceInformation}, potentially applying the content to an
	 * object of the given id.
	 * 
	 * @param information
	 *            must not be {@literal null}.
	 * @param request
	...
	private Object read(RootResourceInformation information, IncomingRequest request,
		...
		if (request.getServerHttpRequest().getMethod().equals(HttpMethod.PUT)
				|| (request.isPatchRequest() && converter instanceof MappingJackson2HttpMessageConverter)) {
